        ...
        if line.split() != []:
            if line.split()[0] not in out_dict:
                if (line.split()[0] in multiple_element):
                    out_dict[line.split()[0]] = [[float(i)
                                                  for i in line.split()[2:]]]
                else:
                    out_dict[line.split()[0]] = line.split()[1:]
            else:
                out_dict[line.split()[0]].append([float(i)
                                                  for i in line.split()[2:]])
    content_file.close()
    return out_dict
...
def get_objects(data):
    ...
    for item in data['SPHERE']:
        ...
        sphere['scale'] = np.matrix(
            [[item[3], 0, 0], [0, item[4], 0, ], [0, 0, item[5]]])
        sphere['trans'] = np.matrix([[item[3], 0, 0, item[0]], [0, item[4], 0, item[1]], [
                                    0, 0, item[5], item[2]], [0, 0, 0, 1]])
...
def raytrace(origin, direction, depth, objects, background, lights, camera, reflection, default_ambient):
    ...
    closest_object, min_distance = closest_intersected_object(
    ...
    # apply ambient
    ambient = closest_object['color'] * \
        closest_object['ambient'] * default_ambient
    illution += ambient
    ...
    for light in lights:
        ...
        _, near_object_distance = closest_intersected_object(
            objects, shifted_point, light_direction)
        ...
        if not is_shadowed:
            ...
            diffuse = lightDotNormal * \
                ...
                closest_object['diffuse']*light['color']
            specular = pow(max(np.dot(
                R, V), 0.0), closest_object['specexp']) * closest_object['specular'] * light['color']  # //; //Phong
            if (np.dot(L, N) < 0.0):
    ...
    if reflection != 0:
        ...
        re_direction = direction-2 * \
            np.dot(direction, normal_to_surface)*normal_to_surface
        color += raytrace(re_origin, re_direction, depth+1, objects,
                          background, lights, camera, reflection, default_ambient)
    return color
...
def main():
    ...
    for x in range(width):
        for y in range(height):
            ...
            depth = 0
            reflection = 1
            color = raytrace(origin, direction, depth, objects,
                             background, lights, camera, reflection, ambient)
